package com.example.interest1.database

import android.net.Uri
import com.example.interest1.R
import com.example.interest1.models.CommonModel
import com.example.interest1.models.UserModel
...
fun initFirebase() {
    ...
    REF_DATABASE_ROOT =
        FirebaseDatabase.getInstance("https://interest-d50eb-default-rtdb.europe-west1.firebasedatabase.app/").reference
    USER = UserModel()
    CURRENT_UID = AUTH.currentUser?.uid.toString()
    REF_STORAGE_ROOT = FirebaseStorage.getInstance("gs://interest-d50eb.appspot.com").reference
}
...
fun clearChat(id: String, function: () -> Unit) {
    REF_DATABASE_ROOT.child(NODE_MESSAGES).child(CURRENT_UID).child(id)
        .removeValue()
        .addOnFailureListener { showToast(it.message.toString()) }
        .addOnSuccessListener { REF_DATABASE_ROOT.child(NODE_MESSAGES).child(id).child(CURRENT_UID).removeValue()
            .addOnSuccessListener { function() }
            .addOnFailureListener { showToast(it.message.toString()) }
        }
}
